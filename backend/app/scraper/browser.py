...
class BrowserManager:
    ...
    async def _reset_browser_state(self, *, stop_playwright: bool) -> None:
        if self._browser is not None:
            try:
                ...
                pass
            self._browser = None
        if stop_playwright and self._playwright is not None:
            try:
    ...
    async def _ensure_browser(self) -> Browser:
        async with self._launch_lock:
            if self._browser is not None and self._is_browser_connected():
                return self._browser

            if self._browser is not None and not self._is_browser_connected():
                logger.warning("Detected disconnected browser instance. Relaunching Chromium.")
                await self._reset_browser_state(stop_playwright=False)
            ...
            if self._browser is None:
                ...
                # Keep Chromium flags minimal and Docker-safe; avoid unstable single-process mode.
                launch_args = [
                    "--no-sandbox",
                ...
                if self._browser is None:
                    await self._reset_browser_state(stop_playwright=True)
                    raise RuntimeError(
        ...
        *,
        ...
        try:
            ...
            if not self._is_target_closed_error(exc):
                raise
            logger.warning(
                "Browser context creation failed due closed/disconnected browser; relaunching.",
                error=str(exc),
            )
            await self._reset_browser_state(stop_playwright=False)
            browser = await self._ensure_browser()
